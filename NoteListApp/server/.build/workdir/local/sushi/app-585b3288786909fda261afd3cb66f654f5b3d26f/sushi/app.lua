...
function app.TasklistDatabase:forContext(ctx)
	...
	self.db = _g.jk.mysql.MySQLDatabase:forConnectionStringSync(ctx, "localhost:ariel:ariel:ariel:ssl:utf8")
...
function app.TasklistApiHandler.TaskRequest:setId(value)
	self._id = value
	do return self end
end

function app.TasklistApiHandler.TaskRequest:getId()
	do return self._id end
end
...
function app.TasklistApiHandler:initRoutes()
	...
	do self:addRoute("POST", "/task", function(req, resp, vars)
		...
		do
			...
			do task:setId(requestData:getId()) end
